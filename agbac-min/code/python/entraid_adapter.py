"""
AGBAC-Min EntraID (Azure AD) Adapter Module

This module implements the EntraID-specific adapter for AGBAC-Min dual-subject authorization.
...
class EntraIDAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, any]):
        ...
        logger.info(
            ...
            extra={
                ...
                'app_id_hash': self._sanitize_identifier(self.app_id),
    ...
    def _load_private_key(self, key_path: str) -> None:
        ...
        logger.info(
            ...
            extra={'key_path_hash': self._hash_url(key_path), 'event': 'entraid_key_loaded'}
        )
    ...
    def request_token(
        ...
        try:
            # Step 1: Get agent token from EntraID
            agent_token_data = self._request_agent_token()
            
            # Step 2: Create act assertion
            act_assertion = self._create_act_assertion(act)
            
            # Step 3: Return both components
            ...
            logger.info(
                "EntraID hybrid token request successful",
                extra={
                    'has_agent_token': True,
                    'has_act_assertion': True,
                    'expires_in': token_response.expires_in,
                    'event': 'entraid_token_request_success'
                }
            )
            ...
            logger.error(
                ...
                extra={'error_type': e.__class__.__name__, 'event': 'entraid_unexpected_error'}
            )
    ...
    def _create_act_assertion(self, act: Dict[str, str]) -> str:
        ...
        act_assertion = jwt.encode(payload, self._app_private_key, algorithm='RS256')
        
        logger.debug(
            "Act assertion created",
            extra={
                'jti_hash': self._sanitize_identifier(jti),
                'exp': now + 300,
